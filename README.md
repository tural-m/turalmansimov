# Hi, I'm Tural Mansimov 👋

## 🧑‍💻 About Me
Data & Salesforce professional based in Victoria, BC, Canada.
I build end-to-end analytics projects — from raw data modelling 
in Snowflake to executive dashboards in Power BI.

Currently holding:
- 🏅 Salesforce Certified Administrator (ADM-201)
- 🏅 Salesforce Certified Platform Developer
- 🏅 Salesforce Certified Data Cloud Consultant

## 🛠️ Tech Stack
| Category        | Tools                                      |
|-----------------|--------------------------------------------|
| Cloud Data      | Snowflake, Medallion Architecture          |
| AI / ML         | Snowflake Cortex AI (Sentiment, Complete)  |
| Visualisation   | Power BI, DAX, Tableau                     |
| Languages       | SQL, SOQL, Python (basic)                  |
| CRM             | Salesforce Admin                           |
| Other           | Jira, Confluence, Agile, Git               |

## 📂 Featured Projects

| Project | Tools | Description |
|---------|-------|-------------|
| [Supply Chain Analytics](link) | Snowflake · Power BI | Medallion architecture + 6-page exec dashboard |
| [Healthcare Analytics](link) | Snowflake · Power BI | [brief description] |
| [Sales Analytics](link) | Snowflake · Power BI | [brief description] |
| [Cortex AI Sentiment Analysis](link) | Snowflake Cortex · Power BI | NLP sentiment on 1000 support tickets |

## 📫 Connect
www.linkedin.com/in/tural-m
